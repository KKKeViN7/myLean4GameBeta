...
Statement (n : ℕ) (A : Matrix (Fin n) (Fin n) ℝ) (x : (Fin n) → ℝ) (μ : ℝ)
  (hx : x ≠ 0) (h : A *ᵥ x = μ • x) :
    HasEigenvalue (Matrix.toLin' A) μ := by
      have H : HasEigenvector (Matrix.toLin' A) μ x := by
        rw [HasEigenvector]
        apply And.intro
        ...
        exact h
        exact hx
      exact hasEigenvalue_of_hasEigenvector H
...
--NewTactic ring
NewTheorem Module.End.hasEigenvalue_of_hasEigenvector
--NewDefinition
